...
void write_info_to_events_file(const char * text, InitInfo* init_info, MessageType type)
{
    ...
    if (type == STARTED)
    {
        length = sprintf(buffer, text, get_lamport_time(), init_info->process_id,
                         getpid(), getppid(), init_info->bank_account);
    } else if (type == DONE)
    {
        length = sprintf(buffer, text, get_lamport_time(), init_info->process_id, init_info->bank_account);
    }
...
void write_transfer_info_to_events_file(const char * text, TransferOrder* order, MessageType type)
{
    ...
    if (type == TRANSFER)
    {
        length = sprintf(buffer, text, get_lamport_time(), order->s_src, order->s_amount, order->s_dst);
    } else if (type == DONE)
    {
        length = sprintf(buffer, text, get_lamport_time(), order->s_dst, order->s_amount, order->s_src);
    }
    write(events_log_file, buffer, length);
}
